<?php 
include_once 'repositorio_usuarios.inc.php';


class ValidadorLogin{
	private $usuario;
	private $error;
	function __construct($email, $clave, $conexion){
       $this->error="";
       //verfifico si el email y la clave  no están vacias
       if (!$this->variable_iniciada($email) || !$this->variable_iniciada($clave)){
       	$this->usuario=null;
       	// si estan vacio muestro el siguiente mensaje
       	$this->error="Debes introducir tu eamil y tu contraseña";

       }else{
       	$this->usuario=repositorio_usuarios::obtener_usuario_por_email($conexion, $email);
       	if (is_null($this->usuario) || !password_verify($clave, $this->usuario->getPassword())){
       		//si el usurio es null  y si la contraseña no esta iniciada 
       		$this->error="Datos incorrectos";
       	  }
       }
	}
	 private function variable_iniciada($variable) {
        // verifico si la variable esta iniciada y si no esta vacia
        if (isset($variable) && !empty($variable)) {
            return true;
        } else {
            return false;
        }
    }
    public function obtener_usuario(){
    	return $this->usuario;
    }
    public function obtener_error(){
    	return $this->error;
    }
    public function mostrar_error(){
    	if($this->error !==''){
    		echo "<<br><div class='alert alert-danger' rol='alert'>";
    		echo $this->error;
    		echo "</div><br>";
    	}
    }
}
 ?>